...
void cpArbiterApplyImpulse_SSE(cpArbiter *arb)
{
    ...
    __m128 surface_vr;
    surface_vr = _MM_LOADL_PI(_mm_setzero_ps(), &arb->surface_vr);
    ...
    while(i--){
        ...
        __m128 jtMax = _mm_mul_ss(friction, jn_jbn);
        __m128 jtOld;
        jtOld = _MM_LOADL_PI(jtOld, &(con->jtAcc));
